...
const TicketScannerDashboard = () => {
  ...
  const [scanHistory, setScanHistory] = useState<
    Array<{ code: string; result: TicketResult; timestamp: Date }>
  >([]);
  ...
  const downloadLogs = () => {
    const csvContent = [
      ...
      ...scanHistory.map(
        (scan) =>
          `${scan.timestamp.toISOString()},${scan.code},${
            scan.result.used === false
              ? "Valid"
              : scan.result.used === true
              ? "Used"
              : "Invalid"
          },${scan.result.message},${scan.result.eventId || ""},${
            scan.result.buyerId || ""
          }`
      ),
    ].join("\n");
